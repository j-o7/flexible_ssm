...
from kinetics_dataloader import KineticsDL, CustomBatchSampler
from COIN_loader import COINDL
from breakfast_loader import BkfstDL as BreakfastDL
...
# parse command line arguments for training and eval
def get_args():
    ...
    #Flexibility
    parser.add_argument('--flexible', action='store_true', default=False,
                        help='Temporal flexible training')
    parser.add_argument('--spatial_flex', action='store_true', default=False,
                        help='Spatio-Temporal flexible training (static patch size, flexible image size (div by 16) and pos embeds)')
    parser.add_argument('--flexivit', action='store_true', default=False,
                        help='replicate FlexiViT training (static image size, flex patch size + pos embeds)')
    parser.add_argument('--flex_all', action='store_true', default=False,
                        help='Full Spatio-Temporal flexible training (flex image size, patch size, and pos embeds)')
    parser.add_argument('--static_tokens', action='store_true', default=False,
                        help='Spatio-Temporal flexible training (flex image size and patch size such that  pos embeds are static)')
    ...
    parser.add_argument('--data_set', default='Kinetics', choices=[
        ...
        'mitv1_sparse', 'LVU', 'COIN', 'Breakfast', 'diving',
...
# set up data, model, and run training and testing
def main(args, ds_init):
    ...
    if 'kinetics' in args.data_path:
        print('training kinetics')
        dataset_train, args.nb_classes = KineticsDL('train', 8, args.flexible, args.spatial_flex, args.flexivit, args.flex_all, args.static_tokens), 400
        if args.disable_eval_during_finetuning:
            dataset_val = None
        else:
            # dataset_val, _ = build_dataset(is_train=False, test_mode=False, args=args)
            dataset_val = KineticsDL('test', 8, args.flexible, args.spatial_flex, args.flexivit, args.flex_all, args.static_tokens)
    elif 'COIN' in args.data_path:
        print('training coin')
        dataset_train, args.nb_classes = COINDL('train', num_frames=args.num_frames, resolution=224), 180
        if args.disable_eval_during_finetuning:
            dataset_val = None
        else:
            # dataset_val, _ = build_dataset(is_train=False, test_mode=False, args=args)
            dataset_val = COINDL('test', num_frames=args.num_frames, resolution=224)
    elif 'Breakfast' in args.data_path:
        print('training breakfast')
        dataset_train, args.nb_classes = BreakfastDL('train', num_frames=args.num_frames, resolution=224), 10
        if args.disable_eval_during_finetuning:
            dataset_val = None
        else:
            # dataset_val, _ = build_dataset(is_train=False, test_mode=False, args=args)
            dataset_val = BreakfastDL('test', num_frames=args.num_frames, resolution=224)
    elif 'UCF' in args.data_path:
        print('training UCF')
        dataset_train, args.nb_classes = UCFDL('train', num_frames=args.num_frames, resolution=224, stflex=args.static_tokens), 101
        if args.disable_eval_during_finetuning:
            dataset_val = None
        else:
            # dataset_val, _ = build_dataset(is_train=False, test_mode=False, args=args)
            dataset_val = UCFDL('test', num_frames=args.num_frames, resolution=224)
    elif 'HMDB' in args.data_path:
        print('training HMDB')
        dataset_train, args.nb_classes = HMDBDL('train', num_frames=args.num_frames, resolution=224), 51
        if args.disable_eval_during_finetuning:
            dataset_val = None
        else:
            # dataset_val, _ = build_dataset(is_train=False, test_mode=False, args=args)
            dataset_val = HMDBDL('test', num_frames=args.num_frames, resolution=224)

    elif 'diving' in args.data_path:
        dataset_train, args.nb_classes = DivingDL('train', num_frames=args.num_frames, resolution=224), 48
        if args.disable_eval_during_finetuning:
            dataset_val = None
        else:
            dataset_val = DivingDL('test', num_frames=args.num_frames, resolution=224)

    elif 'SSV2' in args.data_path:
        dataset_train, args.nb_classes = SmthSmthDL('train', num_frames=args.num_frames, resolution=224), 174
        if args.disable_eval_during_finetuning:
            dataset_val = None
        else:
            dataset_val = SmthSmthDL('test', num_frames=args.num_frames, resolution=224)

    elif 'lvu' in args.data_path.lower():
        dataset_train, args.nb_classes = build_dataset(is_train=True, test_mode=False, args=args)
    ...
    if args.flexible and (args.spatial_flex or args.flex_all):
        args.batch_size = 2
        sampler_train = CustomBatchSampler(
            dataset_train.data, batch_size=args.batch_size, num_replicas=num_tasks, rank=global_rank, shuffle=True, spatial_flex=True, static_tokens=False
        )
    elif args.flexible and args.static_tokens:
        args.batch_size = 4
        sampler_train = CustomBatchSampler(
            dataset_train.data, batch_size=args.batch_size, num_replicas=num_tasks, rank=global_rank, shuffle=True,
            spatial_flex=False, static_tokens=True
        )
    elif args.flexible and args.flexivit:
        args.batch_size = 3
        sampler_train = CustomBatchSampler(
            dataset_train.data, batch_size=args.batch_size, num_replicas=num_tasks, rank=global_rank, shuffle=True,
            spatial_flex=False, static_tokens=False
        )
    elif args.flexible:
        args.batch_size = 8
        sampler_train = CustomBatchSampler(
            dataset_train.data, batch_size=args.batch_size, num_replicas=num_tasks, rank=global_rank, shuffle=True, spatial_flex=args.spatial_flex, static_tokens=args.static_tokens  # ambiguous
        )
    else:
    ...
    if 'deit' in args.model:
        ...
        model = create_model(
            ...
            spatial_flex=args.spatial_flex,
            flexivit=args.flexivit,
            flex_all=args.flex_all,
            static_tokens=args.static_tokens
        )

    elif 'mvit' in args.model:
    ...
    if args.finetune:
        ...
        if '.' in args.finetune[:-4] and 'best' not in args.finetune:
            from collections import OrderedDict
    ...
    if 'mvit' not in args.model:
        ...
        num_layers = 16 #hardcoded for mvit base, small as 10
    ...
    if 'mvit' not in args.model:
        ...
        skip_weight_decay_list = {'pos_encoding'}
...
#
    ...
    if args.eval:
        ...
        if 'kinetics' in args.data_path:
            torch.save(model.state_dict(), f'checkpoints/st-static-tokens_epoch_{epoch}_{test_stats["acc1"]}.pt')
        elif 'COIN' in args.data_path:
            torch.save(model.state_dict(), f'checkpoints/base_coin_epoch_{epoch}_{test_stats["acc1"]}.pt')
        if max_accuracy < test_stats["acc1"]:
            max_accuracy = test_stats["acc1"]
            if args.output_dir and args.save_ckpt:
                utils.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                    loss_scaler=loss_scaler, epoch=epoch, model_name='flex_best', model_ema=model_ema)
...
#                    f.write(json.dumps(log_stats) + "\n")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        if args.distributed and not args.flexible:
        ...
        train_stats = train_one_epoch(
            ...
            no_amp=args.no_amp, bf16=args.bf16, args=args
        )
        if args.output_dir and args.save_ckpt:

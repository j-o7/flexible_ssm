...
class VideoClsDataset_sparse(Dataset):
    ...
    def __init__(self, anno_path, prefix='', split=' ', mode='train', clip_len=8,
        ...
        import pandas as pd
        cleaned = pd.read_csv(self.anno_path, header=0, delimiter=self.split)
        # Original arrays
        dataset_samples = list(cleaned.values[:, 2])
        label_array = list(cleaned.values[:, 0])

        # Filter while keeping correspondence
        filtered_samples = []
        filtered_labels = []

        for x, label in zip(dataset_samples, label_array):
            path = f"{self.prefix}{x}.mp4"
            if os.path.exists(path):
                filtered_samples.append(f"{x}.mp4")
                filtered_labels.append(label)

        self.dataset_samples = filtered_samples
        self.label_array = filtered_labels

        print(self.dataset_samples[:10], self.label_array[:10])
    ...
    def loadvideo_decord(self, sample, chunk_nb=0):
        ...
        try:
            ...
            print("video cannot be loaded by decord: ", fname, e)
            return []

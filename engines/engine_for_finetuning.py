...
# run a forward pass and compute classification loss
...
# read the current loss scale from the optimizer
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    num_training_steps_per_epoch=None, update_freq=None, no_amp=False, bf16=False, args=None):
    model.train(True)
    ...
    if loss_scaler is None:
        ...
        optimizer.zero_grad()
    if args.flexivit:
        model.module.flexivit = True
    elif args.flex_all:
        model.module.flex_all = True
    elif args.spatial_flex:
        model.module.spatial_flex = True
    elif args.static_tokens:
        model.module.static_tokens = True
    ...
    # for data_iter_step, (samples, targets) in enumerate(tqdm(data_loader)):
        ...
        if loss_scaler is None:
            ...
            if loss_scaler != 'none':
                ...
                # print('1 temp gradient: ', model.module.patch_embed.proj.weight.grad.shape)
...
def validation_one_epoch(data_loader, model, device, amp_autocast, ds=True, no_amp=False, bf16=False, maxk=5):
    ...
    model.module.flexivit = False
    model.module.flex_all = False
    model.module.static_tokens = False
...
@torch.no_grad()
# run final testing and write per video predictions
...
# merge prediction files from all ranks and compute accuracy
...
# average clip scores for one video and compute its accuracy
